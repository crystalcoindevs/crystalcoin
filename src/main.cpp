...
const string strMessageMagic = "CrystalCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    int64_t nSubsidy = 1;
     ...
     else if(pindexBest->nHeight+1 <= 1100)
     {
        nSubsidy = 100 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 21100)
     {
        nSubsidy = 75 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 41100)
     {
        nSubsidy = 50 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 51100)
     {
        nSubsidy = 40 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 64430)
     {
        nSubsidy = 30 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 65430)
     {
        nSubsidy = 100 * COIN;
        return nSubsidy + nFees;
     }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CrystalCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CrystalCoin launching 6th August";
        CTransaction txNew;
        txNew.nTime = 1407295146;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407295146;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2027126;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x291e00ccd35a159c8cfeeaad13a8b01dc77cf55bbc207a3dbb04e8477b08fc47"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x62, 0x06, 0x12, 0x34 };
